...
class Dataset:
    ...
    def gen_random_rays_at(self, img_idx, batch_size):
        ...
        # Get color and mask from the images
        color = self.images[img_idx][(pixels_y, pixels_x)]  # batch_size, 3
        mask = self.masks[img_idx][(pixels_y, pixels_x)]    # batch_size, 3

        # Construct ray directions in homogeneous coordinates
        p = torch.stack([pixels_x, pixels_y, torch.ones_like(pixels_y)], dim=-1).float().to(device)  # batch_size, 3
